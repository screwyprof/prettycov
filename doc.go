// Package prettycov reads the coverage profile `go test -coverprofile` writes and reports how much
// of each package is covered, which the profile itself does not say.
//
...
//
// It reads the profile and nothing else: no source tree, no go.mod. So it works on a CI artefact or
// on a repository that is not checked out. Profiles are untrusted input for that reason — a path in
// one reaches a terminal, and the renderers replace any rune a terminal would obey.
//
// # Measuring
//
// [Measure] does all of it, applying a profile's rename and exclusions in the one order that is
...
// its own renderer. Both writers return the destination's error, so a report that failed to write
// is not reported as drawn.
//
